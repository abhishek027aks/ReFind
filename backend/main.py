...
from sqlalchemy.exc import IntegrityError as SQLAlchemyIntegrityError, InvalidRequestError
...
TRUSTED_HOSTS = [
    host.strip()
    for host in os.environ.get("REFIND_TRUSTED_HOSTS", "localhost,127.0.0.1,testserver").split(",")
    if host.strip()
]
@asynccontextmanager
...
def validate_configuration():
    if not (DATABASE_URL.startswith("sqlite:///") or IS_POSTGRES):
        raise RuntimeError("REFIND_DATABASE_URL must use sqlite:/// or postgresql+psycopg://")
    if ENVIRONMENT == "production":
        ...
        if not TRUSTED_HOSTS or "*" in TRUSTED_HOSTS:
            raise RuntimeError("REFIND_TRUSTED_HOSTS must list production API hostnames")
        if not IS_POSTGRES:
...
class SQLAlchemyConnection:
    ...
    def _statement(sql, params):
        if "INSERT OR IGNORE" in sql.upper():
            sql = re.sub(r"INSERT\s+OR\s+IGNORE", "INSERT", sql, count=1, flags=re.IGNORECASE)
        ...
        def replace(_match):
            ...
            index += 1
            return f":{name}"
        return text(re.sub(r"\?", replace, sql)), {f"p{i}": value for i, value in enumerate(values)}

    def execute(self, sql, params=()):
        is_insert = sql.lstrip().upper().startswith("INSERT")
        statement, values = self._statement(sql, params)
        if is_insert and " RETURNING " not in sql.upper():
            statement = text(f"{statement.text} RETURNING id")
        result = self.connection.execute(statement, values)
        lastrowid = None
        if is_insert:
            inserted = result.fetchone()
            if inserted:
                lastrowid = inserted[0]
        try:
            if lastrowid is None and result.inserted_primary_key:
                lastrowid = result.inserted_primary_key[0]
        except (AttributeError, InvalidRequestError):
            pass
        return SQLAlchemyResult(result, lastrowid)
...
def connection():
    ...
    return SQLAlchemyConnection(sqlalchemy_engine.connect())

def now(): return datetime.now(timezone.utc).isoformat()
def row(item):
    if not item:
        return None
    return dict(item._mapping) if hasattr(item, "_mapping") else dict(item)
...
def health():
    with closing(connection()) as db:
        summary = {
            "status": "ok",
            "storage": "postgresql" if IS_POSTGRES else "sqlite",
...
def register(payload: RegisterInput):
    with closing(connection()) as db:
        ...
        except (sqlite3.IntegrityError, SQLAlchemyIntegrityError): raise HTTPException(409,"An account with that email already exists")
...
def create_report(payload: ReportInput, authorization: str | None = Header(None)):
    ...
    with closing(connection()) as db:
        ...
        report = row(db.execute("SELECT * FROM reports WHERE id=?",[report_id]).fetchone()); opposite = [row(item) for item in db.execute("SELECT * FROM reports WHERE kind != ?",[payload.kind]).fetchall()]; matches=[]
